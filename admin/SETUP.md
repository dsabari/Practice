...
## Pre Nov-20th
  ...
  * Create Git repository for the 10 teams
    ...
    * `git clone --bare git@github.com:HexaInnovLab/ftp-fork-me.git ftp12`
    * `cd ftp12`
    ...
    * For each team, nn = 01 to 10
      ...
      * cd ftpnn
      * For windows
        * use VS Code, find/replace in files to do the following: replacing ftp12 -> ftpnn, FTP12 -> FTPnn
        * Rename directory restservice/leavemanager/src/main/java/com/hexaware/ftp12/ to restservice/leavemanager/src/main/java/com/hexaware/ftpnn/
      * For Mac OS
        * NOTE: replace nn with the team-id in the following three commands
        * `find . -type f -not -path '*/\.*' -not -path "*/node_modules/*" -exec sed -i '' -e 's/ftp12/ftp[nn]/g' {} \; -exec sed -i '' -e 's/FTP12/FTP[nn]/g' {} \;`
        * `mv restservice/leavemanager/src/main/java/com/hexaware/ftp12/ restservice/leavemanager/src/main/java/com/hexaware/ftp[nn]/`
        * `mv restservice/leavemanager/src/test/java/com/hexaware/ftp12/ restservice/leavemanager/src/test/java/com/hexaware/ftp[nn]/`
        * `mv restservice/integration-test/src/test/java/com/hexaware/ftp12 restservice/integration-test/src/test/java/com/hexaware/ftp[nn]`
  ...
  * Databases - 3 per team
    * Integration and Staging instances
      ...
      * 
    * `CREATE DATABASE FTP12;` -- XX from 01 to 10
    * `CREATE USER 'FTP12'@'%' IDENTIFIED BY 'FTP12';`
    * `GRANT ALL ON FTP12.* TO 'FTP12'@'%';`
    * `select Host, User, Password from mysql.user order by user;`
    * `select * from mysql.db order by Db;`
  * Jenkins jobs - 3 per team
    * FTP12-10-UNIT
    * FTP12-30-INTEGRATION
    * FTP12-50-STAGING
    * For each team, nn = 01 to 10
      * Create a tab FTPnn
