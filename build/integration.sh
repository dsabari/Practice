#!/bin/bash -ex
#deploy
curl --fail -T ftp12.war "http://manager:manager@$INTEGRATION_HOST:8080/manager/text/deploy?path=/ftp12&update=true"
...
while true
do
  response=$(curl --write-out %{http_code} --silent --output /dev/null http://$INTEGRATION_HOST:8080/ftp12/favicon.ico)
